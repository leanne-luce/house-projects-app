...
- **Server-side tesseract.js for receipt OCR**, replacing the browser-side tesseract.js call — same free engine, moved into a Node server function so it isn't dependent on a CDN load succeeding in whatever browser Leanne happens to be using, and so it can retry on failure. **Load-tested, not just assumed** (Phase 4): ~350ms warm / ~700ms cold for a full receipt-sized image, comfortably inside any serverless function budget. One real gotcha found and fixed along the way: tesseract.js spawns an actual Node `worker_threads` worker at a file path on disk, which breaks under Next.js's default bundling — fixed with `serverExternalPackages: ["tesseract.js"]` in `next.config.ts`, which tells Next.js to require it straight from `node_modules` at runtime rather than bundling it. The manual "add item by hand" fallback stays exactly as designed, since this still isn't going to be perfect.
...
6. **Stretch, sequenced by value (post-MVP, revisit priority with Leanne before starting)**:
   ...
   - Pinterest integration — **decided and built** (moved up from a checkpoint): Leanne chose a best-effort server-side scrape of the linked board's public page over the official OAuth API (no developer account needed, works today) — see `src/lib/pinterest.ts` for exactly how, and its caveats. This is intentionally not the official API: it's outside Pinterest's terms of service for automated access and can break if Pinterest changes their page markup, in exchange for zero setup. Mood board and Reference & assembly are also merged into one visual section (`references-panel.tsx`) with a per-image toggle to reclassify between the two, rather than being two separate collections the user has to pick between up front.
...
## Test/real data isolation

**The incident:** before this section existed, local development had exactly one Postgres database (`house_projects_dev`) and one uploads folder (`.uploads/`), used for both real day-to-day use and ad-hoc manual testing. A round of test cleanup ran `rm -rf .uploads/*` to clear out test-upload artifacts — but since real uploads landed in that same folder, the command deleted 9 of Leanne's 11 real uploaded files (six progress photos, a screenshot, a real Lowe's receipt PDF, one more image) with no backup available to recover them. The database rows for those assets survived, so the Detail page kept referencing files that no longer existed on disk. This should never have been possible: test cleanup should physically be unable to touch real data, regardless of what command gets typed.

**The fix — full physical separation, not just care:**

- **Separate database.** `house_projects_test` (created via `createdb`) alongside `house_projects_dev`. Never share one.
- **Separate uploads folder.** `.uploads-test/`, selected via a `UPLOAD_DIR` env var read by both `src/lib/storage.ts` and `src/app/api/uploads/[...path]/route.ts` (must be kept in sync between the two — same env var, same default of `.uploads`). Wiping `.uploads-test/` freely is always safe; `.uploads/` never gets touched by test cleanup.
- **Separate env file.** `.env.test.local` sets `DATABASE_URL` to the test database and `UPLOAD_DIR=.uploads-test`, plus its own throwaway `SESSION_SECRET`/`APP_PASSPHRASE`. Next.js's own env-loading convention (`NODE_ENV=test` loads `.env.test.local` and explicitly skips `.env.local`) does the switching — no extra dependency needed.
- **Separate npm scripts.** `npm run dev:test` (`NODE_ENV=test next dev -p 3311`, a different port so it can run alongside the real dev server) and `npm run db:push:test` (`drizzle-kit push` against `drizzle.test.config.ts`).
- **A hard runtime guard in `drizzle.test.config.ts`**: it throws if `DATABASE_URL` isn't set, and throws again if the URL doesn't contain `_test` — so even a misconfigured `.env.test.local` can't push schema changes at the real database by accident.
- **A Claude Code PreToolUse hook** (`.claude/hooks/guard-real-data.mjs`, wired in the committed `.claude/settings.json`) that inspects every Bash command before it runs and hard-blocks the exact incident pattern: a blanket `rm -rf`/`find -delete` targeting bare `.uploads` or `.uploads/*` (a specifically named file inside it, e.g. `rm .uploads/one-file.jpg`, stays allowed — that's sometimes a legitimate deliberate cleanup), plus `dropdb`/`DROP DATABASE`/`TRUNCATE`/unscoped `DELETE FROM` (no `WHERE`) against `house_projects_dev` specifically. It does not restrict anything scoped to the test database or `.uploads-test`. This is project-wide (checked into git via `.claude/settings.json`), not a personal setting, so it protects any session working in this repo, not just the one that built it.

**Working with this going forward:** any testing that creates, uploads, or deletes data should run against `npm run dev:test` (port 3311) with `house_projects_test`/`.uploads-test`, never against the real dev server. If a task genuinely requires verifying something against real data, prefer read-only inspection (page loads, `SELECT`s) over any write/delete, and if a write is unavoidable, scope it to specific IDs rather than a blanket operation.

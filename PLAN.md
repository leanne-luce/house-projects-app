...
- **Neon Postgres** — matches the brief's own suggestion; serverless HTTP driver pairs well with Vercel's serverless functions (no connection-pool management); confirmed free tier (0.5GB storage / 100 CU-hrs/mo) comfortably covers a single-user app's row volume (no images live in Postgres — those go to Blob).
